...
resource "aws_alb_target_group" "port8081-tg-3" {
  name     = format("%s-%s-port8081-tg-3",var.dns_name, var.account_environment)
  port     = 8081
  protocol = "HTTP"
  target_type = "ip"
  vpc_id   = "${aws_vpc.main.id}" 
...
resource "aws_alb_target_group" "port8082-tg-4" {
  name     = format("%s-%s-port8082-tg-4",var.dns_name, var.account_environment)
  port     = 8082
  protocol = "HTTP"
  target_type = "ip"
  vpc_id   = "${aws_vpc.main.id}" 
...
resource "aws_alb_listener_rule" "rule-2" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_alb_target_group.port8081-tg-3.arn
  }
...
resource "aws_alb_listener_rule" "rule-3" {
  listener_arn = aws_alb_listener.alb-listener-443.arn
  action {
    type             = "forward"
    target_group_arn = aws_alb_target_group.port8082-tg-4.arn
  }
