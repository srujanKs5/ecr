...
resource "aws_alb_target_group" "port81-tg-3" {
  name     = format("%s-%s-port80-tg-1",var.dns_name, var.account_environment)
  port     = 81
  protocol = "HTTP"
  target_type = "ip"
  ...
  tags = {
    Name = format("%s-%s-port80-tg-3",var.dns_name, var.account_environment) 
  }
}


resource "aws_alb_target_group" "port8081-tg-4" {
  name     = format("%s-%s-port8081-tg-4",var.dns_name, var.account_environment)
  port     = 8081
  protocol = "HTTP"
  target_type = "ip"
  vpc_id   = "${aws_vpc.main.id}" 
...
resource "aws_alb_listener" "alb-listener-81" {
  load_balancer_arn = aws_alb.main-ext-lb.arn
  port = "81"
  protocol = "HTTP"
  default_action {
...
resource "aws_alb_listener" "alb-listener-444" {
  load_balancer_arn = aws_alb.main-ext-lb.arn
  port = "444"
  protocol = "HTTPS"
  certificate_arn = var.alb_certificate
...
resource "aws_alb_listener_rule" "rule-2" {
  listener_arn = aws_alb_listener.alb-listener-444.arn
  priority     = 1
  action {
    type             = "forward"
    target_group_arn = aws_alb_target_group.port8081-tg-4.arn
  }
